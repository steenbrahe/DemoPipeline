...
./xatestercli.sh -e simulator -f . --recursive -G -S COBOL -s https://localhost:8447 -u XATUSER -p 123456 -x -g TestResults'''
        ...
        xaTester(environmentId: 'simulator', folderPath: '.', cliPath: '/opt/xatester/cli', xaTesterServerUrl: 'https://localhost:8447', recursive: true, copyReportsToReportFolder: true, reportFolder: 'TestResults', sonarVersion: 'V6', sourceFolder: 'COBOL', uploadToServer: true, credentialsId: 'hostuser')
      }
    }
    stage('Deploy') {
      parallel {
